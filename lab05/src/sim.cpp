...
int main(int argc, char **argv) {
    ...
    if (opt.disasm != NULL) {
        u32 lo, hi;
        parse_range(opt.disasm, "--disasm", &lo, &hi);
        for (u32 addr = lo & ~3u; addr < hi; addr += 4) {
            u32 raw = ram_read(m.ram, addr, 4);
            printf("0x%08x: %08x    %s\n", addr, raw, disasm(raw, addr).c_str());
        }
    }

    Processor cpu;
    processor_init(cpu, cfg, st, *m.imem, *m.dmem);
    if (!opt.norun) {
        processor_run(cpu);
    } else {
        log_info("--norun: the program was loaded but not executed");
    }
