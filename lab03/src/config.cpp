...
void config_defaults(Config &cfg) {
    ...
    // Not a performance figure: purely a bound on a runaway program. A correct
    // program returns from main and exits through tohost long before this.
    ...
    // pipeline; resolved in ID, one. Those are the counts the 2025 labs used
    // for the no-forwarding and forwarding models respectively, and they match
    // what the lectures cover.
    cfg.pipelined_resolve  = RESOLVE_MEM;
    cfg.forwarding_resolve = RESOLVE_ID;
    // Frozen at max in Phase E: [I1_Cache] and [L1_Cache] are separate arrays,
    // and serialising a fetch against a data access is the cost that splitting
    // the L1 exists to remove. See config.ini for the full argument and for
    // what max knowingly ignores (contention at the shared L2 on a coincident
    // miss). It is also what the 2025 Python simulator charged.
    ...
    // cycle counts that depend only on the pipeline, which is what weeks 3-9
    // want. The shipped config.ini turns the hierarchy on for weeks 10-12.
