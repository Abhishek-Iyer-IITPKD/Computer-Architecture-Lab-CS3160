...
#
# The reference solutions exercise the instructions a student writes; they do
# not go near division by zero, the one signed-division overflow, the direction
# signed division rounds in, or the difference between a signed and an unsigned
# comparison of the same two bit patterns. Those are exactly the places the
# previous Python simulator was wrong -- it used Python's floor division, which
# rounds -7 / 2 to -4 where RISC-V truncates to -3, and it implemented none of
# the special cases at all.
#
